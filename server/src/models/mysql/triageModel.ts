...
export class TriageModel {
  ...
  static async getAllTriagesInAscendentOrderById(): Promise<ITriage[]> {
    const query = `SELECT * FROM Triage ORDER BY id ASC`;
    const conn = await connect();
    const [rows] = await conn.query<ITriage[]>(query);
    return rows;
  }
  ...
  static async update(
    triage: { level: string; color: string },
    currentLevel: string
  ): Promise<ITriage> {
  ...
  static async updateById(id: number, triage: { level: string; color: string }): Promise<void> {
    const query = `UPDATE Triage SET level = ?, color = ? WHERE id = ?`;
    const conn = await connect();
    await conn.query(query, [triage.level, triage.color, id]);
  }
