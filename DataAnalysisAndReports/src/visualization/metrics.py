...
def mean(txt_y, df, y):
    return f'La media de {txt_y} ingreasdos es de {round(calculate_mean(df, y),2)}'

def min(txt_x, txt_y, df, x, y):
    return f'La {txt_x} que menos {txt_y} se ingresaron fue el {calculate_min_x(df, x, y)} con {calculate_min(df, y)} {txt_y}'
