...
def cols_rename(df):
    ...
    return df.rename(columns=cols_rename, inplace=True)
...
def triage_level_to_integer(df):
    ...
    df=df['TRIAGE'].replace(new_values)
...
def isolated_to_boolean(df):
    ...
    df = df['AISLADO'].map(new_values)
    df = df[df['AISLADO']!='724']
    return df['AISLADO'].astype(bool)

def generating_alta_col(df):
    df = df[~df['DESTINO'].isin['?','INT','HMD/ 315','HMD',' ', '    ','  ',]]
    df['ALTA'] = df['DESTINO'].str.contains('alta|obito|traslado|derivacion|AL. VOL|DERIVAC',case=False,regex=True)
    return df['ALTA'].fillna(True,inplace=True)