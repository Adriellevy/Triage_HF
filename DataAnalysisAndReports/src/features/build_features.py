...
def calculate_age(born: datetime) -> int:
    return (
        dt.date.today().year
        - born.year
        - ((dt.date.today().month, dt.date.today().day) < (born.month, born.day))
    )
...
def date_filters(df: DataFrame, dictionary: Dict[str, Any]) -> DataFrame:
    if dictionary["from_"] is None and dictionary["to"] is None:
        ...
        dictionary["from_"] = pd.to_datetime(
            dictionary["from_"], format="%Y-%m-%d", errors="coerce"
        )
        dictionary["to"] = pd.to_datetime(
            dictionary["to"], format="%Y-%m-%d", errors="coerce"
        )

    df = df[
        (df["patient_entry_time"] >= dictionary["from_"])
        & (df["patient_entry_time"] <= dictionary["to"])
    ]
...
def triage_level_style(df: DataFrame) -> pd.Series:
    df["patient_triage_level"] = df["patient_triage_level"].apply(
...
def build_number_patients_date(
    df: DataFrame, group_cond: str, rename: Dict[Any, str] = None
) -> DataFrame:
    df["patient_entry_time"] = df["patient_entry_time"].dt.date

    df = (
        df.groupby(["patient_entry_time", group_cond], sort=False)
        .size()
...
def build_top_queries_date(
    df: DataFrame,
    group_cond: str,
    rename: Dict[Any, str] = None,
    top: int = 10,
    order: str = "",
) -> DataFrame:
    df = (
        df.groupby(["patient_symptom", group_cond])
        .size()
        .reset_index(name="symptom_count")
    )

    top_reasons = (
        df.groupby("patient_symptom")["symptom_count"].sum().nlargest(top).index
    )
    ...
    for symptom in df["patient_symptom"].unique():
        for triage_level in all_triage_levels:
            all_combinations.append(
                {"patient_symptom": symptom, group_cond: triage_level}
            )

    all_combinations_df = DataFrame(all_combinations)

    merged_df = pd.merge(
        all_combinations_df, df, on=["patient_symptom", group_cond], how="left"
    )
    merged_df["symptom_count"].fillna(0, inplace=True)

    merged_df = merged_df.sort_values(
        by=[group_cond, "symptom_count"], ascending=[True, False]
    )

    if order == "asc":
        merged_df = merged_df.sort_values(
            by=["patient_symptom"],
            key=lambda x: x.map(dict(zip(top_reasons[::-1], range(len(top_reasons))))),
        ).reset_index(drop=True)
    else:
        merged_df = merged_df.sort_values(
            by=["patient_symptom", group_cond],
            key=lambda x: x.map(dict(zip(top_reasons, range(len(top_reasons))))),
        ).reset_index(drop=True)
...
def build_patients_mean_time(
    df: DataFrame, group_cond1: str, group_cond2: str
) -> DataFrame:
    ...
    df = (
        df.groupby([group_cond1, group_cond2])["patient_delta_time"]
        .mean()
        .reset_index(name="patient_mean_delta_time")
    )

    df["patient_mean_delta_time"] = df["patient_mean_delta_time"].apply(
...
async def build_features(
    table_name: str, dictionary: Dict[str, Any], condition: str = ""
) -> Union[DataFrame, None]:
    pd.options.display.max_rows = None
    pd.options.display.max_columns = None

    print("llego a build_features")
    if condition == "":
        where_dictionary = filter_dictionary(
            dictionary,
            [
                "patient_age",
                "patient_isolated",
        ...
        join_dictionary = filter_dictionary(
            dictionary, ["doctor_full_name", "nurse_full_name", "box_type"]
        )
