...
df.loc[~fecha_vacia, 'FECHA'] = pd.to_datetime(df.loc[~fecha_vacia, 'FECHA'] + '-2024', format='%d-%m-%Y', errors='coerce')
...
def generar_hora_salida(hora_ingreso):
    # Tiempo máximo de estancia hasta 72 horas (3 días)
...
df['HORA SALIDA'] = df['HORA INGRESO'].apply(lambda x: generar_hora_salida(x) if pd.notna(x) else '')
...
df.to_csv('planilla_con_horarios.csv', index=False)
