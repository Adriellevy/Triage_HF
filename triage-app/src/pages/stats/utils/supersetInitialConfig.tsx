...
async function supersetLogin() {
  ...
  async function getDashboards(accessToken: string) {
    const DASHBOARDS_URL = 'http://127.0.0.1:8088/api/v1/dashboard/'
    try {
        const response = await fetch(DASHBOARDS_URL, {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${accessToken}`
          }
...
//   FUNCION PRINCIPAL PARA REUTILIZAR
  export async function createGuestToken() {
    ...
    if (!csrfToken) {
        console.error('Error: No se obtuvo dashboards de getDashboards');
        throw new Error('Missing dashboards');
      }
