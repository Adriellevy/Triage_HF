  ...
  useEffect(() => {
    const handleResize = () => {
      if (sidebarRef.current) {
        ...
        if (window.innerWidth <= 767) {
        ...
        } else if (window.innerWidth <= 1015 && window.innerWidth >= 767) {
          ...
          sidebarRef.current.style.position = 'fixed'
          sidebarRef.current.style.bottom = '0'
          sidebarRef.current.style.left = '22%'
        } else {
  ...
  return (
    <div
      ...
      className={`selector-menu-container ${
        sidebarVisible ? 'mx-auto ml-4' : 'fixed bottom-0 w-full pl-5'
    ...
    >
      ...
      >
        {options.map((option, index) => (
          <div
            key={index}
            className={`selector-option flex items-center p-2 mt-2 cursor-pointer transition duration-200 mr-2  ${
