...
export class ShiftService {
    ...
    async getCurrentShift():Promise<ShiftOutputDTO>{
        try{
            const now = new Date();
            const shift = await this.shiftRepository.findCurrentShift(now,now.getHours(),now.getHours()+environment.shift_duration);
    ...
    async getAllShifts(date?:Date):Promise<ShiftOutputDTO[]>{
        try{
            const shifts = await this.shiftRepository.findByOptions(date ? {day:date.toISOString().split('T')[0]}:{})
